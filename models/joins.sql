...
select ct.category_name, sp.company_name as suppliers, pd.product_name, pd.unit_price,
pd.product_id
...
left join {{source('sources', 'categories')}} ct on (pd.category_id = ct.category_id)
    ...
    select ord.order_date, ord.order_id, cs.company_name as customer, em.name as employee, em.age, em.lenghofservice
    ...
    select od.*, ords.order_date, ords.customer, ords.employee, ords.age, ords.lenghofservice
    from orddetai od
    inner join ordrs ords on (od.order_id = ords.order_id)
)
